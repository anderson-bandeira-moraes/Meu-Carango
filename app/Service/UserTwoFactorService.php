...
/**
 * Serviço responsável pela lógica de autenticação em duas etapas (2FA) para lojistas.
 * Gerencia geração, envio, verificação, reenvio e status de códigos 2FA.
 * 
 * Adaptado do TwoFactorService (admin) para usar chaves de sessão com prefixo 'user_'.
 */
class UserTwoFactorService
{
     ...
     *
     * @param string $email E-mail do lojista
    ...
    public function verify(string $email, string $code): array
    {
        try {
            ...
            // Código válido: remove registro e marca verificação concluída (para lojista)
            $this->repository->reset($email);
            $this->session->set('2fa_verified_user', true); // <-- ADAPTADO: chave com prefixo 'user_'
