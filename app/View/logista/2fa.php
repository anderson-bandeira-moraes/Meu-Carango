<div class="row justify-content-center">
    <div class="col-md-6 col-lg-5">
        <div class="card shadow-lg border-0 rounded-4">
            <div class="card-header bg-dark text-white text-center rounded-top-4">
                <h4 class="mb-0"><i class="bi bi-shield-lock-fill me-2"></i>Verificação em Duas Etapas</h4>
            </div>
            <div class="card-body p-4">

                <!-- Mensagens flash -->
                <?php if (!empty($erro)): ?>
                    <div class="alert alert-danger alert-dismissible fade show" role="alert">
                        <i class="bi bi-exclamation-triangle-fill me-2"></i>
                        <?= htmlspecialchars($erro) ?>
                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Fechar"></button>
                    </div>
                <?php endif; ?>

                <?php if (!empty($sucesso)): ?>
                    <div class="alert alert-success alert-dismissible fade show" role="alert">
                        <i class="bi bi-check-circle-fill me-2"></i>
                        <?= htmlspecialchars($sucesso) ?>
                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Fechar"></button>
                    </div>
                <?php endif; ?>

                <!-- Instruções -->
                <p class="text-muted text-center">
                    Um código de 6 dígitos foi enviado para o e-mail:
                    <strong><?= htmlspecialchars($email) ?></strong>
                </p>

                <!-- Status do código -->
                <?php if (isset($status) && $status['exists']): ?>
                    <div class="mb-3 text-center small">
                        <p class="text-muted small">
                            <i class="bi bi-clock me-1"></i>
                            O código é válido por <strong><?= htmlspecialchars($expiryMinutes ?? 5) ?> minutos</strong>.
                        </p>
                    </div>
                <?php endif; ?>

                <!-- Formulário de verificação -->
                <form method="POST" action="/logista/2fa/verify" id="verifyForm">
                    <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">

                    <div class="mb-3">
                        <label for="code" class="form-label">Código de verificação</label>
                        <input type="text"
                               class="form-control form-control-lg text-center"
                               id="code"
                               name="code"
                               placeholder="123456"
                               maxlength="6"
                               pattern="[0-9]{6}"
                               inputmode="numeric"
                               autocomplete="off"
                               autofocus
                               <?php
                               $codeExists = isset($status['exists']) && $status['exists'] === true;
                               $codeExpired = isset($status['expires_at']) && strtotime($status['expires_at']) < time();
                               $formDisabled = !$codeExists || $codeExpired;
                               echo $formDisabled ? 'disabled' : '';
                               ?>
                               required>
                        <div class="form-text text-center">
                            Digite os 6 dígitos recebidos por e-mail.
                        </div>
                    </div>

                    <div class="d-grid gap-2">
                        <button type="submit" class="btn btn-primary btn-lg" id="verifyBtn"
                            <?= $formDisabled ? 'disabled' : '' ?>>
                            <i class="bi bi-check-lg me-2"></i>Verificar
                        </button>
                    </div>
                </form>

                <hr>

                <!-- Reenvio e ações secundárias -->
                <div class="d-flex flex-wrap justify-content-between align-items-center">
                    <form method="POST" action="/logista/2fa/resend" id="resendForm" class="d-inline">
                        <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
                        <button type="submit" class="btn btn-link p-0 text-decoration-none" id="resendBtn">
                            <i class="bi bi-arrow-clockwise me-1"></i>Reenviar código
                        </button>
                    </form>

                    <a href="/logista/logout" class="btn btn-primary" onclick="return confirm('Deseja voltar à página de login?');">
                        <i class="bi bi-box-arrow-left me-1"></i>Login
                    </a>
                </div>

            </div>
        </div>
    </div>
</div>

<!-- MODAL DE BLOQUEIO (exibido apenas quando $showModal for true) -->
<?php if (isset($showModal) && $showModal === true): ?>
<div class="modal fade" id="blockedModal" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header bg-warning text-dark">
                <h5 class="modal-title"><i class="bi bi-clock me-2"></i>Reenvio bloqueado</h5>
            </div>
            <div class="modal-body text-center">
                <p>Você atingiu o limite de reenvios.</p>
                <p>Será redirecionado para a página de login em <span id="countdown">5</span> segundos.</p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" id="cancelRedirectBtn">Cancelar</button>
            </div>
        </div>
    </div>
</div>
<?php endif; ?>

<script>
    (function() {
        'use strict';

        document.addEventListener('DOMContentLoaded', function() {
            // Foco automático no campo de código (se habilitado)
            const codeInput = document.getElementById('code');
            if (codeInput && !codeInput.disabled) {
                codeInput.focus();
            }

            // --- Modal de bloqueio (se $showModal for true) ---
            <?php if (isset($showModal) && $showModal === true): ?>
                const modalElement = document.getElementById('blockedModal');
                if (modalElement) {
                    const modal = new bootstrap.Modal(modalElement, {
                        backdrop: 'static',
                        keyboard: false
                    });
                    modal.show();

                    let seconds = 5;
                    const countdownEl = document.getElementById('countdown');
                    const interval = setInterval(function() {
                        seconds--;
                        if (countdownEl) {
                            countdownEl.textContent = seconds;
                        }
                        if (seconds <= 0) {
                            clearInterval(interval);
                            window.location.href = '/logista/login';
                        }
                    }, 1000);

                    // Botão "Cancelar" – interrompe a contagem e fecha o modal (permite permanecer na página)
                    const cancelBtn = document.getElementById('cancelRedirectBtn');
                    if (cancelBtn) {
                        cancelBtn.addEventListener('click', function() {
                            clearInterval(interval);
                            modal.hide();
                        });
                    }

                    // Caso o modal seja fechado sem o botão "Cancelar" (ex: clicar fora, mas com backdrop estático não permite)
                    // Para segurança, se o modal for escondido sem cancelar, também para a contagem
                    modalElement.addEventListener('hidden.bs.modal', function() {
                        clearInterval(interval);
                    });
                }
            <?php endif; ?>
        });

        // Prevenir envio duplicado do formulário de verificação
        document.getElementById('verifyForm')?.addEventListener('submit', function(e) {
            const btn = document.getElementById('verifyBtn');
            btn.disabled = true;
            btn.innerHTML = '<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Verificando...';
        });

        // Desabilitar temporariamente o botão de reenvio para evitar múltiplos cliques
        document.getElementById('resendForm')?.addEventListener('submit', function(e) {
            const btn = document.getElementById('resendBtn');
            if (btn && !btn.disabled) {
                btn.disabled = true;
                btn.innerHTML = '<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Enviando...';
            }
        });

    })();
</script>